...
num_training = None # Set to None for all training data
...
directory = os.path.join(model_dir, date + '_' + backbone +'_' + ''.join(str(d) for d in dataset) + '_' + str(training_samples) + 'Train' + '_' + str(p) + 'Dropout')
...
if dataparallel:
    net = nn.DataParallel(net, device_ids = [1,2,3,4,5,6,7])
net.to(device)
